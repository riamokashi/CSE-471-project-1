...
CSynthesizer::CSynthesizer() {
    ...
    m_secperbeat = 0.5;
    m_beatspermeasure = 4;
    m_waveFactory.LoadFile("Bottle.wav");
}
...
//
    ...
    while (m_currentNote < (int)m_notes.size())
    {
        ...
        else if (note->Instrument() == L"DrumInstrument")
        {
            ...
            instrument = m_drumFactory.MakeInstrument();
        }
    ...
    // Figure out code for this part to get the effect to play 
    ...
    for (list<CInstrument*>::iterator node = m_instruments.begin(); node != m_instruments.end(); )
    {
        ...
        double flange_frame[2], noise_gate_frame[2], chorus_frame[2];

        for (int i = 0; i < 2; i++)
        {
            flange_frame[i] = 0;
            noise_gate_frame[i] = 0;
            chorus_frame[i] = 0;
        }

        if (channelframes[2][0] != 0)
        {
            m_flange.Process(channelframes[1], flange_frame);
        }
        if (channelframes[2][0] != 0)
        {
            m_noise_gate.Process(channelframes[2], noise_gate_frame);
        }
        if (channelframes[3][0] != 0)
        {
            m_chorus.Process(channelframes[2], chorus_frame);
        }

        for (int i = 0; i < GetNumChannels(); i++)
        {
            frame[i] += frames[i];
            frame[i] += flange_frame[i];
            frame[i] += noise_gate_frame[i];
            frame[i] += chorus_frame[i];
